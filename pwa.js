...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('/sw.js?v=81', { updateViaCache: 'none' }).then(registration => registration.update()).catch(() => {});
  navigator.serviceWorker.addEventListener('controllerchange',()=>{
    if(sessionStorage.getItem('signal_sw_reloaded_v84')==='1')return;
    sessionStorage.setItem('signal_sw_reloaded_v84','1');
    location.reload();
  });
}
...
const SIGNAL_FRONTEND_VERSION='2.4.0',SIGNAL_SW_VERSION='v84',SIGNAL_CACHE_VERSION='signal-shell-v84-profile-experience';
...
window.addEventListener('load', async () => {
  ...
  try {
    ...
    const forgotLabel=lockEnglish?'Forgot PIN?':'Забули PIN?',requestLabel=lockEnglish?'Send code to email':'Надіслати код на email';
    ...
    requestButton.insertAdjacentHTML('beforebegin',`<div id="pinEmailCode" hidden style="margin:12px 0"><p id="pinEmailHint" style="margin:0 0 9px;color:#b7c5df;font-size:12px;line-height:1.5"></p><input id="pinEmailCodeInput" inputmode="numeric" autocomplete="one-time-code" maxlength="6" placeholder="000000" style="box-sizing:border-box;width:100%;padding:13px;text-align:center;letter-spacing:8px;border:1px solid #68d9ff55;border-radius:13px;background:#ffffff0b;color:white;font-size:20px;outline:none"><button id="verifyPinEmailCode" type="button" style="width:100%;margin-top:9px;border:0;border-radius:13px;padding:12px;background:linear-gradient(135deg,#20cdaa,#477cff);color:white;font-weight:800">${lockEnglish?'Confirm code':'Підтвердити код'}</button><button id="resendPinEmailCode" type="button" style="margin-top:8px;border:0;background:transparent;color:#8fa5ca;font-size:12px;font-weight:700">${lockEnglish?'Send a new code':'Надіслати новий код'}</button><p id="pinEmailError" style="min-height:16px;color:#ff8c9b;font-size:12px;margin:8px 0 0"></p></div>`);
    const emailCodeBox=document.getElementById('pinEmailCode');
    let recoveryTimer=null,recoveryDismissed=true;
    const showRecovery=()=>{pinEntry.hidden=true;newPinPanel.hidden=true;recovery.hidden=false;};
    const showPin=()=>{recoveryDismissed=true;recovery.hidden=true;newPinPanel.hidden=true;pinEntry.hidden=false;emailCodeBox.hidden=true;};
    const showNewPin=()=>{recoveryDismissed=false;pinEntry.hidden=true;recovery.hidden=true;newPinPanel.hidden=false;document.getElementById('newPin').focus();};
    const renderResetStatus=request=>{
      ...
      if(request?.status==='completed'){if(recoveryDismissed){showPin();return;}showRecovery();emailCodeBox.hidden=true;requestButton.hidden=false;requestButton.disabled=false;requestButton.textContent=requestLabel;title.textContent=lockEnglish?'PIN recovery':'Відновлення PIN';copy.textContent=lockEnglish?'Send a one-time code to your account email.':'Надішліть одноразовий код на email вашого акаунта.';return;}
      if(request?.status==='pending'){if(!recoveryDismissed)showRecovery();title.textContent=lockEnglish?'Check your email':'Перевірте email';copy.textContent=request.emailCodeSent?(lockEnglish?'Enter the 6-digit code from the email. The administrator can also approve the request.':`Введіть 6-значний код, надісланий на ${request.maskedEmail}. Адміністратор також бачить запит.`):(lockEnglish?'Email is temporarily unavailable. The administrator can approve the request.':'Email тимчасово недоступний. Адміністратор уже бачить запит і може його підтвердити.');emailCodeBox.hidden=!request.emailCodeSent;document.getElementById('pinEmailHint').textContent=lockEnglish?`Code sent to ${request.maskedEmail}`:`Код надіслано на ${request.maskedEmail}`;requestButton.disabled=true;requestButton.hidden=true;return;}
      if(recoveryDismissed){showPin();return;}
      showRecovery();emailCodeBox.hidden=true;requestButton.hidden=false;requestButton.disabled=false;requestButton.textContent=requestLabel;
      if(request?.status==='denied'){title.textContent=lockEnglish?'Request not approved':'Запит не підтверджено';copy.textContent=lockEnglish?'Send a new email code or contact support.':'Надішліть новий код на email або зверніться в підтримку.';}
    ...
    document.getElementById('forgotPin').onclick=()=>{recoveryDismissed=false;showRecovery();requestButton.hidden=false;checkResetStatus();};
    ...
    document.getElementById('verifyPinEmailCode').onclick=async()=>{const code=document.getElementById('pinEmailCodeInput').value.trim(),errorBox=document.getElementById('pinEmailError'),button=document.getElementById('verifyPinEmailCode');errorBox.textContent='';if(!/^\d{6}$/.test(code)){errorBox.textContent=lockEnglish?'Enter all 6 digits.':'Введіть усі 6 цифр.';return;}button.disabled=true;button.textContent=lockEnglish?'Checking…':'Перевіряємо…';try{const response=await fetch(`${API_URL}/api/account/lock/reset-request/verify-code`,{method:'POST',headers:{'Content-Type':'application/json','x-session-token':token},body:JSON.stringify({code})}),data=await response.json();if(!response.ok)throw Error(data.error||'Invalid code');renderResetStatus(data.request);}catch(error){errorBox.textContent=error.message;button.disabled=false;button.textContent=lockEnglish?'Confirm code':'Підтвердити код';}};
    document.getElementById('resendPinEmailCode').onclick=async()=>{const button=document.getElementById('resendPinEmailCode'),errorBox=document.getElementById('pinEmailError');button.disabled=true;errorBox.textContent=lockEnglish?'Sending…':'Надсилаємо…';try{const response=await fetch(`${API_URL}/api/account/lock/reset-request/email-code`,{method:'POST',headers:{'Content-Type':'application/json','x-session-token':token},body:'{}'}),data=await response.json();if(!response.ok)throw Error(data.error||'Send failed');errorBox.style.color='#69e6c2';errorBox.textContent=lockEnglish?'A new code was sent.':'Новий код надіслано.';renderResetStatus(data.request);}catch(error){errorBox.style.color='#ff8c9b';errorBox.textContent=error.message;}finally{button.disabled=false;}};
