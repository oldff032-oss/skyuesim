...
test('travel mode has customer reminders and an authenticated admin overview', () => {
  ...
  assert.match(read('signal-universe.html'), /travel-assistant\.html/);
});
...
test('customer pages render immediately and admin navigation stays compact', () => {
  ...
  const deviceCheck = read('device-check.html');
  assert.doesNotMatch(pwa, /visibility:hidden!important/);
  assert.doesNotMatch(pwa, /classList\.add\('signal-maintenance-check'\)/);
  assert.equal((nav.match(/\['admin-esims\.html'/g)||[]).length, 1);
  assert.match(nav, /renderGroup\('Операції'/);
  assert.match(nav, /renderGroup\('Система'/);
  assert.match(deviceCheck, /getElementById\('confidenceNote'\)\?\.remove\(\)/);
});
...
test('travel planner dates fit mobile cards and secondary tools stay off the home screen', () => {
  ...
  const universe = read('signal-universe.html');
