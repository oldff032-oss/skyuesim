...
function supportMetrics(tickets){
  ...
  const adminEmails=[...new Set(enriched.flatMap(t=>[t.assignedTo,...(t.messages||[]).map(m=>m.adminEmail)].filter(Boolean)))];
  const byAdmin=adminEmails.map(email=>{const handled=enriched.filter(t=>t.assignedTo===email||(t.messages||[]).some(m=>m.adminEmail===email)),responses=handled.map(t=>{const first=(t.messages||[]).find(m=>m.from==='admin'&&m.adminEmail===email);return first?Math.max(0,Math.round((time(first.createdAt)-time(t.createdAt))/60000)):null}).filter(v=>v!=null),resolved=handled.filter(t=>['resolved','closed'].includes(t.status));return {email,handled:handled.length,replies:handled.reduce((sum,t)=>sum+(t.messages||[]).filter(m=>m.from==='admin'&&m.adminEmail===email).length,averageFirstResponseMinutes:responses.length?Math.round(responses.reduce((a,b)=>a+b,0)/responses.length):null,averageResolutionMinutes:resolved.length?Math.round(resolved.reduce((sum,t)=>sum+Math.max(0,(time(t.resolvedAt||t.updatedAt)-time(t.createdAt))/60000),0)/resolved.length):null,overdue:handled.filter(t=>t.overdue).length,reopened:handled.filter(t=>t.reopenedAt).length};}).sort((a,b)=>b.handled-a.handled);
