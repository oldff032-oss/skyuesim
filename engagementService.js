...
function walletCard(user = {}) {
  ...
  const usageStale=Boolean(esim.usageStale),dataStatus=usageStale?'unknown':remainingGb==null?'unlimited':remainingGb<=0?'empty':remainingGb<1?'critical':remainingGb<3?'low':'healthy';
  ...
  return {serial:`signal-${crypto.createHash('sha256').update(user.email||'guest').digest('hex').slice(0,16)}`,holder:text(user.displayName,80)||'Signal Traveler',plan:text(esim.packageName||latest.packageName||esim.plan||user.plan||'eSIM',100),destination:text(trip?.destination||esim.location||latest.location||'Global',80),validUntil,status,usedGb:usageStale?null:cleanNumber(usedGb),totalGb:usageStale?null:cleanNumber(totalGb),remainingGb:usageStale?null:cleanNumber(remainingGb),usagePercent:usageStale?null:cleanNumber(usagePercent),usageStale,usageCounterSource:text(esim.usageCounterSource,40)||null,dataStatus,esimReadiness,tripStartDate,tripEndDate,daysUntilTrip,tripStatus,daysUntilExpiry,lastSyncAt:esim.lastUsageSyncAt||esim.lastUpdateTime||user.updatedAt||null,familyReady,familyTotal:familyMembers.length};
}
