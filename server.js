...
function persistSupportLinkUsage(email,user,usage){
  ...
  const normalized={...usage,source:'support_short_url',counterUpdatedAt:usage.lastUpdateTime||null,hasUsageTimestamp:Boolean(usage.lastUpdateTime)},freshness=providerUsageFreshness(user,normalized);
  if(freshness.stale){const syncedAt=new Date().toISOString();saveUser(email,{esim:{...user.esim,lastUsageSyncAt:syncedAt,usageStale:true,usageChanged:false,lastUsageSyncError:freshness.warning}});return{...cachedEsimUsage(getUser(email),freshness.warning),source:'support_short_url',stale:true,live:false,changed:false,warning:freshness.warning,syncedAt,providerUpdatedAt:freshness.counterUpdatedAt,providerManagedExternally:true};}
  return persistProviderUsage(email,user,normalized,{providerManagedExternally:true});
}

function cachedEsimUsage(user,warning='Оператор тимчасово не оновив дані. Показано останній успішно збережений залишок.'){
  const esim=user?.esim||{},usedBytes=esim.usedBytes!=null?Math.max(0,Math.trunc(Number(esim.usedBytes)||0)):Math.max(0,Math.round(Number(esim.usedGb||0)*(1024**3)));
  const totalBytes=esim.totalBytes!=null?Math.max(0,Math.trunc(Number(esim.totalBytes)||0)):(esim.dataLimitGb==null?null:Math.max(0,Math.round(Number(esim.dataLimitGb)*(1024**3))));
  const remainingBytes=esim.remainingBytes!=null?Math.max(0,Math.trunc(Number(esim.remainingBytes)||0)):(totalBytes==null?null:Math.max(0,totalBytes-usedBytes));
...
function providerUsageFreshness(user,usage){
  const profile=user?.esim||{},cached=cachedEsimUsage(user),candidateUsed=usage.usedBytes==null?null:Math.max(0,Math.trunc(Number(usage.usedBytes)||0)),providerTotal=usage.totalBytes==null?null:Math.max(0,Math.trunc(Number(usage.totalBytes)||0)),pendingTopup=profile.pendingTopupConfirmation||null,expectedTopupTotal=Math.max(0,Math.trunc(Number(pendingTopup?.expectedMinimumTotalBytes)||0)),counterUpdatedAt=providerCounterTime(usage),counterChanged=candidateUsed!==cached.usedBytes||providerTotal!==cached.totalBytes,activityReference=profile.lastTopupAt||profile.activateTime||profile.assignedAt||profile.lastUsageSyncAt||profile.createdAt||null,activityAgeMs=activityReference?Date.now()-new Date(activityReference).getTime():0,counterAgeMs=counterUpdatedAt?Date.now()-new Date(counterUpdatedAt).getTime():Infinity,lastSyncAgeMs=profile.lastUsageSyncAt?Date.now()-new Date(profile.lastUsageSyncAt).getTime():Infinity,inUse=String(usage.esimStatus||profile.esimStatus||'').toUpperCase()==='IN_USE',topupNotConfirmed=Boolean(pendingTopup)&&(expectedTopupTotal>0?(providerTotal==null||providerTotal<expectedTopupTotal):!counterChanged),authoritativeCounter=['realtime_usage_api','share_usage_api'].includes(String(usage.source||'')),counterIsActuallyOld=counterUpdatedAt?counterAgeMs>6*60*60*1000:lastSyncAgeMs>6*60*60*1000,unchangedOldCounter=!authoritativeCounter&&inUse&&activityAgeMs>6*60*60*1000&&!counterChanged&&counterIsActuallyOld,stale=Boolean(usage.stale||topupNotConfirmed||unchangedOldCounter),warning=topupNotConfirmed?'Пакет прийнято, але оператор ще не підтвердив новий загальний обсяг.':'Оператор повернув лише старий резервний лічильник. Дані з окремого API трафіку очікуються.';
  return{cached,candidateUsed,providerTotal,counterUpdatedAt,counterChanged,stale,warning};
}
function persistProviderUsage(email,user,usage,extra={}){
  const profile=user.esim||{},usedNumber=Number(usage.usedBytes),totalNumber=Number(usage.totalBytes);
  if(!Number.isFinite(usedNumber)||!Number.isFinite(totalNumber)||usedNumber<0||totalNumber<=0||usedNumber>totalNumber)throw Object.assign(new Error('Оператор повернув некоректний лічильник трафіку.'),{code:'PROVIDER_USAGE_INVALID'});
  let usedBytes=Math.trunc(usedNumber),totalBytes=Math.trunc(totalNumber),remainingBytes=Math.max(0,totalBytes-usedBytes);
  const providerState=String(usage.esimStatus||profile.esimStatus||'IN_USE').trim().toUpperCase(),expiredTime=usage.expiredTime??profile.expiredTime??null,expiryMs=expiredTime?new Date(expiredTime).getTime():NaN,providerUsedUp=['USED_UP','EXHAUSTED','DEPLETED'].includes(providerState),providerExpired=providerState==='EXPIRED'||(Number.isFinite(expiryMs)&&expiryMs<=Date.now());
  if(providerUsedUp){usedBytes=totalBytes;remainingBytes=0;}else if(providerExpired)remainingBytes=0;
  const previous=cachedEsimUsage(user),syncedAt=usage.syncedAt||new Date().toISOString(),counterUpdatedAt=providerCounterTime(usage),counterChanged=usedBytes!==previous.usedBytes||totalBytes!==previous.totalBytes,usedGb=usedBytes/(1024**3),totalGb=totalBytes/(1024**3),remainingGb=remainingBytes/(1024**3),day=syncedAt.slice(0,10),history=[...(profile.usageHistory||[])],source=usage.source||'profile_api',snapshot={day,usedBytes,totalBytes,remainingBytes,usedGb,totalGb,remainingGb,source,providerUpdatedAt:counterUpdatedAt};
  ...
  const exhausted=providerUsedUp||(totalBytes>0&&remainingBytes===0),serviceEnded=exhausted||providerExpired,serviceEndedReason=providerExpired?'expired':exhausted?'used_up':null,nextStatus=providerExpired?'expired':exhausted?'used_up':(profile.status==='pending'?'active':profile.status||'active'),fingerprint=`${profile.iccid||''}:${usedBytes}:${totalBytes}:${providerState}:${counterUpdatedAt||''}`;
  const nextEsim={...profile,usedBytes,totalBytes,remainingBytes,usedGb,dataLimitGb:totalGb,remainingGb,apn:usage.apn??profile.apn,expiredTime,activateTime:usage.activateTime??profile.activateTime,lastUsageSyncAt:syncedAt,lastProviderUsageAt:counterUpdatedAt,usageSource:source,usageCounterSource:usage.counterSource||null,usageStale:false,usageChanged:counterChanged,lastUsageSyncError:null,providerUsageFingerprint:fingerprint,usageHistory:history.slice(-31),status:nextStatus,esimStatus:providerExpired?'EXPIRED':exhausted?'USED_UP':providerState,serviceEnded,serviceEndedReason,pendingTopupConfirmation:null};
  saveUser(email,{status:user.status==='blocked'?'blocked':'active',esim:nextEsim});refreshGoogleWallet(email);
  return{usedBytes,totalBytes,remainingBytes,usedGb,totalGb,remainingGb,source,stale:false,live:usage.live===true,changed:counterChanged,...extra,esimStatus:nextEsim.esimStatus,serviceEnded,serviceEndedReason,apn:nextEsim.apn||null,expiredTime:nextEsim.expiredTime||null,activateTime:nextEsim.activateTime||null,lastUpdateTime:counterUpdatedAt,providerUpdatedAt:counterUpdatedAt,syncedAt};
}
...
async function syncEsimUsageForUser(email,{force=false}={}){
  ...
  const profile=user.esim;
  if(!force&&profile.lastUsageSyncAt&&Date.now()-new Date(profile.lastUsageSyncAt).getTime()<25*60*1000)return cachedEsimUsage(user,'Показано останні дані. Наступна автоматична перевірка виконується раз на 30 хвилин.');
  if(profile.provider==='support-link'||String(profile.orderNo||'').startsWith('SUPPORT-')){
    if(!profile.supportInstallUrl)return cachedEsimUsage(user,'Для автоматичної перевірки потрібне повне посилання підтримки. Показано останні збережені дані.');
    try{return persistSupportLinkUsage(email,user,await checkSupportLinkUsage(profile.supportInstallUrl));}
    catch(error){
      const syncedAt=new Date().toISOString();
      saveUser(email,{esim:{...profile,lastUsageSyncAt:syncedAt,usageStale:true,lastUsageSyncError:error.message}});
  ...
  if(freshness.stale){
    ...
    saveUser(email,{esim:{...profile,lastUsageSyncAt:syncedAt,usageStale:true,usageChanged:false,lastUsageSyncError:freshness.warning}});
    return{...freshness.cached,source:usage.source||freshness.cached.source,stale:true,live:false,changed:false,warning:freshness.warning,syncedAt,providerUpdatedAt:freshness.counterUpdatedAt};
  }
  return persistProviderUsage(email,user,usage);
}
...
const ESIM_USAGE_SYNC_INTERVAL_MS=30*60*1000;
let usageSyncCycleRunning=false;
async function syncAllActiveEsimUsage(){
  if(usageSyncCycleRunning)return;usageSyncCycleRunning=true;
  let checked=0,updated=0,stale=0,failed=0;
  try{
    const candidates=Object.entries(getAllUsers()).filter(([,user])=>user?.esim&&user.status!=='blocked'&&Boolean(user.esim.orderNo||user.esim.esimTranNo||user.esim.iccid||user.esim.supportInstallUrl));
    for(const [email] of candidates){
      checked+=1;
      try{const result=await syncEsimUsageForUser(email);if(result.stale)stale+=1;else updated+=1;}
      catch(error){
        failed+=1;const user=getUser(email);if(user?.esim)saveUser(email,{esim:{...user.esim,lastUsageSyncAt:new Date().toISOString(),usageStale:true,lastUsageSyncError:error.message}});
        console.error('[esim usage scheduler]',email,error.message);
      }
    }
    if(checked)await storage.saveNow('users.json',getAllUsers());
    console.log('[esim usage scheduler]',{checked,updated,stale,failed});
  }finally{usageSyncCycleRunning=false;}
}
...
app.get('/api/admin/esims',adminAuth.requireAdmin,adminAuth.requirePermission('users.read'),(req,res)=>{
  ...
  const knownUsers=new Map([...Object.keys(authStore.readAll().users||{}),...Object.keys(getAllUsers())].map(email=>[String(email).trim().toLowerCase(),true]));
  const eligibleUsers=[...knownUsers.keys()].map(email=>{const user=getUser(email)||{};return {email,displayName:user.displayName||null,status:user.status||'registered',hasEsim:Boolean(user.esim)};}).filter(user=>user.status!=='blocked'&&!user.hasEsim).sort((a,b)=>a.email.localeCompare(b.email));
  ...
  res.json({records,eligibleUsers,assignmentEvents,totals:{all:records.length,available:records.filter(item=>item.state==='available').length,inUse:records.filter(item=>['active','installed','suspended'].includes(item.state)).length,attention:records.filter(item=>['unknown','quarantined'].includes(item.state)).length,finished:records.filter(item=>['expired','used_up','cancelled','revoked'].includes(item.state)).length}});
});
...
app.post('/api/admin/esims/sync-provider',adminAuth.requireAdmin,adminAuth.requirePermission('esim.retry'),async(req,res)=>{
  ...
  try{
    ...
    for(const profile of profiles){
      ...
      const stateOverride=existing?.stateOverride==='replaced'?'replaced':state==='available'?null:['revoked','cancelled'].includes(state)?state:'quarantined';
      const pooledRecord={...(existing?.source==='pool'?existing:{}),id,profile,plan:existing?.plan||'custom',packageName:existing?.packageName||profile.packageName||null,purchaseId:existing?.purchaseId||null,previousOwnerEmail:existing?.ownerEmail||existing?.previousOwnerEmail||null,stateOverride,storedAt:existing?.storedAt||new Date().toISOString()};
      putEsimInPool(pooledRecord);
      known.set(id,{...pooledRecord,source:'pool'});pooled+=1;if(stateOverride==='quarantined')review+=1;
    }
...
app.post('/api/admin/esims/:id/replace-and-assign',adminAuth.requireAdmin,adminAuth.requireRole('super_admin'),adminAuth.requirePermission('esim.manage',{requireTwoFactor:true}),async(req,res)=>{
  const id=String(req.params.id||''),targetEmail=String(req.body?.email||'').trim().toLowerCase(),record=findEsimInventoryRecord(id);
  if(!record||!['pool','current'].includes(record.source))return res.status(404).json({error:'Профіль у списку провайдера не знайдено'});
  const state=esimInventory.profileState(record.profile,record.stateOverride),replaceable=['deleted_from_device','revoked','expired','used_up','cancelled','quarantined'].includes(state);
  if(!replaceable||record.replacementIssuedAt)return res.status(409).json({error:'Для цього запису заміну вже видано або вона не потрібна'});
  if(req.body?.confirmProviderCharge!==true)return res.status(400).json({error:'Підтвердіть створення нової eSIM за рахунок балансу eSIM Access'});
  const sourceOwner=record.source==='current'?getUser(record.ownerEmail):null;
  if(record.source==='current'&&record.ownerEmail===targetEmail)return res.status(409).json({error:'Для передачі вкажіть іншого користувача. Для заміни власнику спочатку перенесіть старий профіль до архіву.'});
  const target=getUser(targetEmail);
  if(!target&&!authStore.readAll().users?.[targetEmail])return res.status(404).json({error:'Користувача-отримувача не знайдено'});
  if(target?.status==='blocked')return res.status(409).json({error:'Акаунт користувача заблоковано'});
  ...
  try{
    ...
    if(sourceOwner){
      const sourceHistory=[...(sourceOwner.esimHistory||[])];
      sourceHistory.unshift({plan:sourceOwner.plan||null,esim:{...record.profile,lifecycleState:'replaced'},transferredAt:now,transferredTo:targetEmail,reason:'admin_replacement_transfer',purchaseId:record.purchaseId});
      saveUser(record.ownerEmail,{esim:null,status:sourceOwner.status==='blocked'?'blocked':'esim_transferred',esimHistory:sourceHistory.slice(0,20),lastEsimAdminActionAt:now});
      const sourcePurchase=purchaseForProfile(sourceOwner,record.profile);
      if(sourcePurchase)upsertPurchase(record.ownerEmail,sourcePurchase.id,{esimLifecycleState:'replaced',esimTransferredAt:now,esimTransferredTo:targetEmail});
    }
    saveUser(targetEmail,{email:targetEmail,status:'active',plan,esim:{...profile,plan,packageName:record.packageName||profile.packageName||null,location:record.profile?.location||profile.location||null,dataLimitGb:record.profile?.dataLimitGb??profile.dataLimitGb??null,durationDays:record.profile?.durationDays??profile.durationDays??null,inventoryProfileId:newProfileId,grantType:'admin_replacement',priceCents:0,assignedAt:now,assignedBy:req.admin.email},esimGrants:[grant,...(target?.esimGrants||[])].slice(0,100),lastEsimProvisionError:null});
    putEsimInPool({...record,ownerEmail:null,previousOwnerEmail:record.ownerEmail||record.previousOwnerEmail||null,stateOverride:'replaced',replacementIssuedAt:now,replacementProfileId:newProfileId,replacementAssignedTo:targetEmail,replacementOrderNo:profile.orderNo||null});
    await storage.saveNow('users.json',getAllUsers());
    await operationsStore.saveNow();
...
app.post('/api/admin/referrals/:email/credit-reward', adminAuth.requireAdmin, adminAuth.requireRole('super_admin', 'admin'), async (req, res) => {
  ...
  try {
    ...
    saveUser(inviter.email, { esim:mergeTopupUsage(inviter.esim,topup,{packageCode},new Date().toISOString()) });
...
app.post('/api/admin/users/:email/resync-esim', adminAuth.requireAdmin, adminAuth.requireRole('super_admin', 'admin'), adminAuth.requirePermission('esim.retry'), async (req, res) => {
  const email = String(req.params.email||'').trim().toLowerCase();
  const user = getUser(email);
  if (!user?.esim) return res.status(404).json({ error: 'Активну eSIM не знайдено' });
  try {
    const result=await syncEsimUsageForUser(email,{force:true});
    await storage.saveNow('users.json',getAllUsers());
    auditStore.log({adminEmail:req.admin.email,action:'esim_usage_resynced',target:email,details:{source:result.source,stale:result.stale,usedBytes:result.usedBytes,totalBytes:result.totalBytes,remainingBytes:result.remainingBytes}});
    const remaining=result.remainingGb==null?'безліміт':`${result.remainingGb.toFixed(2)} GB`;
    const message=result.serviceEnded?(result.serviceEndedReason==='expired'?'Оператор підтвердив: строк дії пакета завершився.':'Оператор підтвердив: пакет повністю використано, залишок 0 GB.'):result.stale?'Оператор не передав новий лічильник. Цифри не оновлено і вони не показуються як актуальні.':result.changed?`Отримано новий лічильник оператора. Залишок: ${remaining}.`:`Оператор підтвердив лічильник без змін. Залишок: ${remaining}.`;
    res.json({ok:true,...result,message});
  } catch (error) {
    recordDiagnostic(req,{email,source:'esim_access',type:'esim_usage',action:'admin_sync',outcome:'failed',severity:'warning',message:error.message,errorCode:error.code||'USAGE_SYNC_FAILED'});
    res.status(Number(error.status)>=400?Number(error.status):502).json({error:error.message,code:error.code||'USAGE_SYNC_FAILED'});
  }
});
...
app.post('/api/admin/users/:email/esim-topups',adminAuth.requireAdmin,adminAuth.requireRole('super_admin'),adminAuth.requirePermission('esim.manage',{requireTwoFactor:true}),requireProviderCapacity,rateLimit('admin_esim_topup',60*60*1000,20,req=>req.admin.email),async(req,res)=>{
  ...
  try{
    ...
    res.json({ok:true,message:'Пакет прийнято eSIM Access. Новий QR-код не потрібен, але гігабайти з’являться лише після нового лічильника оператора. Локальний перерахунок вимкнено.',topup:{packageCode,packageName:selected.name,dataLimitGb:selected.dataLimitGb,durationDays:selected.durationDays,providerConfirmationPending:true,expiredTime:nextEsim.expiredTime||null}});
  }catch(error){
...
app.patch('/api/admin/users/:email/esim-usage',adminAuth.requireAdmin,adminAuth.requireRole('super_admin'),adminAuth.requirePermission('esim.manage',{requireTwoFactor:true}),async(req,res)=>{
  res.status(410).json({error:'Ручне редагування трафіку вимкнено. Цифри може змінювати лише лічильник оператора.'});
});
...
app.post('/api/admin/users/:email/purchases/:purchaseId/retry-provision', adminAuth.requireAdmin, adminAuth.requireRole('super_admin','admin'), adminAuth.requirePermission('esim.retry'), async (req, res) => {
  ...
  if (purchase.kind === 'esim_topup' || purchase.changeMode === 'topup_existing') {
    ...
    try {
      ...
      const esim=mergeTopupUsage(user.esim,topup,{packageCode:purchase.packageCode,dataLimitGb:purchase.dataLimitGb,unlimited:purchase.unlimited},new Date().toISOString());
      saveUser(email,{status:'active',esim});
      refreshGoogleWallet(email);
...
async function processSubscriptionRenewal(invoice) {
  ...
  try {
    ...
    saveUser(user.email, {
      ...
      esim:mergeTopupUsage(current.esim,topup,{packageCode:selected.packageCode,dataLimitGb:selected.dataLimitGb,unlimited:selected.unlimited},now),
    });
    ...
    if(changeMode==='topup_existing'){
      ...
      if(existingPurchase?.fulfillmentStatus!=='provisioned'){
        ...
        try{
          ...
          const esim=mergeTopupUsage(current.esim,topup,{packageCode,dataLimitGb:Number(session.metadata.dataLimitGb)||null,unlimited:session.metadata.unlimited==='true'},new Date().toISOString());
...
app.get('/api/usage', requireUserSession, async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    if(cachedUser?.esim)return res.json(cachedEsimUsage(cachedUser));
...
storage.init().then(() => Promise.all([
  ...
  const initialUsageSyncTimer=setTimeout(()=>syncAllActiveEsimUsage().catch(error=>console.error('[esim usage scheduler]',error.message)),15*1000);
  initialUsageSyncTimer.unref?.();
  const usageSyncTimer=setInterval(()=>syncAllActiveEsimUsage().catch(error=>console.error('[esim usage scheduler]',error.message)),ESIM_USAGE_SYNC_INTERVAL_MS);
